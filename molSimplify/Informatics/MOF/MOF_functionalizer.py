...
def functionalize_MOF(cif_file, path2write, functional_group = 'F', functionalization_limit = 1, path_between_functionalizations = 3, additional_atom_offset = 0):
    ...
    dict_approach, gram_schmidt = check_support(functional_group)
   ...
   # Change
    ...
    if bond_overlap:
        ...
        with open(f'bond_overlap_{functionalization_limit}_func.txt', 'a') as file:
...
def first_functionalization(cell_v, molcif,
    ...
    molcif, atom_types_to_add, additions_to_cart, functionalization_counter, functionalized = apply_functionalization(cell_v, molcif, 
                                        ...
                                        linker_cart_coords, linker_to_functionalize, linker_atom_types, linker_graph, functionalization_counter, gram_schmidt, additional_atom_offset=additional_atom_offset)
            ...
            for l, secondary_connected_atom in enumerate(secondary_connected_atom_types):
                if (secondary_connected_atom == 'H') and (not functionalized):                      
                    molcif, atom_types_to_add, additions_to_cart, functionalization_counter, functionalized = apply_functionalization(cell_v, molcif, 
                                    ...
                                    functionalization_counter, gram_schmidt, additional_atom_offset=additional_atom_offset)
...
def apply_functionalization(cell_v, molcif, allatomtypes, position_to_functionalize, atom_to_replace, position_to_functionalize_neighbors, 
                                        functional_group, linker_cart_coords, linker_to_analyze, linker_atom_types, linker_graph, functionalization_counter, gram_schmidt, additional_atom_offset=0):
    #######################################################################################################
    ...
    if not gram_schmidt:
        ...
        if len(connection_atom_dict[functional_group])>1: 
            ...
            while unintend_bond:
                atom_types_to_add_new = atom_types_to_add.copy()
                additions_to_cart_new = additions_to_cart.copy()

                molcif, atom_types_to_add_new, additions_to_cart_new = multiatomic_functionalization(connection_atom_dict, 
                    bond_length_dict, 
                    bond_angle_dict, 
                    bond_rotation_dict,
                    functional_group,
                    ...
                    norm_cp,
                    initial_placement,
                    additions_to_cart_new,
                    atom_types_to_add_new, 
                    molcif,
                    update_molcif=False,
                    additional_atom_offset=angle
                    )
               

                fcoords_updated = frac_coord(np.concatenate((molcif.coordsvect(), additions_to_cart_new[1:,:])), cell_v)
                atom3d_atomtypes_new = molcif.getAtoms()
                allatomtypes_new = [atom.symbol() for atom in atom3d_atomtypes_new]
                allatomtypes_new.extend(atom_types_to_add_new[1:])
            

                new_coord_list, final_atom_types = atom_deletion(np.array(fcoords_updated), allatomtypes_new, [atom_to_replace])
                #molcif_new.deleteatom(atom_to_replace)
                # Adding atoms (the atoms in the functional groups)
                #allatomtypes_new, fcoords_new = atom_addition(atom_types_to_add_new, final_atom_types, new_coord_list, additions_to_cart_new, cell_v)
                
                bond_overlap = post_functionalization_overlap_and_bonding_check(cell_v, final_atom_types, new_coord_list, atom_types_to_add_new)

                if not bond_overlap or angle >= 360:
        ...
        # connection_length_dict, connection_atom_dict, bond_length_dict, bond_angle_dict, bond_rotation_dict = geo_dict_loader()
        with open('additional_functional_group.pkl', 'rb') as pickle_file:
        ...
        while unintend_bond:
            ...
            molcif, atom_types_to_add, additions_to_cart = functionalization_gram_schmidt(molcif, geo_dict_loader_gram_schmidt, 
...
def functionalization_gram_schmidt(molcif, geo_dict_loader_gram_schmidt, functional_group, 
    ...
    for i in range(no_func_group_atoms):
        coefficients = geo_dict_loader_gram_schmidt[functional_group]['gram_schmidt_coeff'][0]
        new_coords.append(coefficients[i][0]*e1 + coefficients[i][1]*e2 + coefficients[i][2]*e3)

    #new_coords = np.array(new_coords) + np.array(linker_cart_coords[functionalization_position_on_linker])
    print(e1)
...
def check_support(functional_group):
    ...
    """
    supported_functional_groups = ['F', 'Cl', 'Br', 'I', 'CH3', 'CN', 'NH2', 'NO2', 'CF3', 'OH', 'SH', 'COOH']
    supported_functional_groups_gram_schmidt = ['1-3-5-7-octatetraene', '1-3-5-7-9-11-dodecapentaene', 'acetamide', 'penta-1-3-diene', 'styrene', 
    ...
    if functional_group not in supported_functional_groups+supported_functional_groups_by_mol3D_merge+supported_functional_groups_gram_schmidt:
        raise ValueError('Unsupported functional group requested.')
    else:
        dict_approach = functional_group in supported_functional_groups+supported_functional_groups_gram_schmidt
        gram_schmidt = False
        if functional_group in supported_functional_groups_gram_schmidt:
            gram_schmidt = True
        return dict_approach, gram_schmidt
...
def post_functionalization_overlap_and_bonding_check(cell_v, allatomtypes, fcoords, extra_atom_types):
    ...
    cart_coords = fractional2cart(fcoords, cell_v)
    distance_mat = compute_distance_matrix3(cell_v, cart_coords)
    adj_matrix, _ = compute_adj_matrix(distance_mat, allatomtypes, handle_overlap=True) # Will throw an error if atoms are overlapping after functionalization.
    adj_matrix = adj_matrix.todense()
    ...
    num_func_group_atoms = len(flattened_extra_atom_types)
...
def main():
    # Functional groups that are supported currently
    func_group = ['F','Cl','CN','CH3','CF3','NH2','NO2','OH','SH', 'COOH', 'Br', 'I',
                  'acetamide', 'propanamine', 'propyl', 'pyrrole', 'ethyl', 'methyl-amine', 
                  'acetylene', 'ethyl-amine', 'propyne', 'sulfonic'] 
    
    mofname = os.listdir('./cifs_additional/') ## Location to CIFs to be functionalized 
    print(f'Total no of MOFs to be functionalized: {len(mofname)}')
    
    mof_path = './cifs_additional/'
    
    
    mof_functionalized = 0
    counter = 0
    for mof in mofname:
        if '.cif' in mof:
    ...
    # Defining folder names
            ...
            base_write_path = './functionalized_final/'+mof_wo_cif+'/'
    ...
    # Functionalization should occur on the primitive cell.
            ...
            try:
                get_primitive(base_database_path+mof, base_database_path_primitive+mof)
            except ValueError:
        ...
        ## Removing overlapping atoms from the MOFs
            mkdir_if_absent(base_database_path_without_overlap)
        ...
        ## End of modification
            ...
            for MOF in cifs_without_overlap:
                ...
                num_list = [1, 2] ## Perform both mono and di functionalization
                path = 3
        ...
        mof_functionalized += 1
        print(f'MOF functionalized: {mof_functionalized}')
