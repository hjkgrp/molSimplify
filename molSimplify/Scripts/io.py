...
from typing import Any, List, Dict, Tuple, Union, Optional, cast
...
def lig_load_safe(userligand: str, licores: Optional[dict] = None) -> Tuple[Any, str]:
    """
    Load a ligand.
    Output currently typed as any instead of Union[mol3D, None] because many other
    scripts depend on a mol3D as first return value.
    ...
    # ----- SAFE handling of licores possibly being None -----
    groups: List[str] = []
    ...
    if licores is not None:
        ...
        # similarity vs dict keys
        keys_list = list(licores.keys())
        text_similarities = [difflib.SequenceMatcher(None, userligand, k).ratio() for k in keys_list]
    ...
    if licores is not None:
        if (userligand in licores) or (text_similarities and max(text_similarities) > 0.6):
            go_in_dict = True

    if go_in_dict:
        # Tell mypy this cannot be None here
        assert licores is not None
        licores_nn: Dict[str, Any] = cast(Dict[str, Any], licores)

        emsg += "dict"
        if userligand in licores_nn:
            print(f'Loading ligand from dictionary: {userligand}')
            dbentry = licores_nn[userligand]
        else:
            # similarity path
            keys_list = list(licores_nn.keys())
            max_similarity = max(text_similarities)
            index_max = text_similarities.index(max_similarity)
            desired_ligand = keys_list[index_max]
            print(f'Ligand was not in dictionary, but the sequence is very similar to a ligand that is: {desired_ligand}')
            print(f'Loading ligand from dictionary: {desired_ligand}')
            dbentry = licores_nn[desired_ligand]
        # Load lig mol file (with hydrogens).
        if globs.custom_path:
            flig = globs.custom_path + "/Ligands/" + dbentry[0]
        else:
